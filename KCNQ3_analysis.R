...
Protein_Length <- 872;  # Plug in the length of KCNQ3 protein sequence

##1-2 Fill the mutation position vector using the information from the database
Inputframe <- read.table("input/KCNQ3_missense position.txt", header=TRUE);  # Copy the mutation positions from this file

Mutant_Position <- Inputframe$AA_POSITION;  # There are 14 different positions for missense mutations
...
write.table(nmc(Input_Data, alpha=alpha, multtest=multiple_test), file="output/result_nmc method_KCNQ3.txt", sep="\t");  # Write the output into a separate file

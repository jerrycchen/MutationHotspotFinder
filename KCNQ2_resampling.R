...
## Dataset preparation
Domain <- read.table("input/domain_coordinates_KCNQ2.txt", header=TRUE);
Position <- read.table("input/missense_position_KCNQ2.txt", header=TRUE)$AA_POSITION;
num.AA <- 872;
num.perm <- 10^5;
Range <- 105:637;  # Identified by previous test using the nmc method
...
write.table(Output.Data, file="output/KCNQ2_resampling details.txt", sep="\t");
write.table(Output.Pval, file="output/KCNQ2_resampling summary.txt", sep="\t");




## Perform analysis within the region [52,651]
## No "Ankyrin_G_Binding" or "Blank2"

## Dataset preparation
Domain <- read.table("input/domain_coordinates_KCNQ2.txt", header=TRUE);
Position <- read.table("input/missense_position_KCNQ2.txt", header=TRUE)$AA_POSITION;
Range <- 52:637;  # Identified by previous test using the nmc method

Range.New <- c(52,651);  #@__@ Specify the region to be analyzed;
...
## Write the simulated outcome into text
write.table(Output.Data, file="output/KCNQ2_resampling details_incluster.txt", sep="\t");
write.table(Output.Pval, file="output/KCNQ2_resampling summary_incluster.txt", sep="\t");